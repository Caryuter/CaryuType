...
function beginTransition(el, type, options, disableCSS = false) {
	if (!isInDOM(el)) {
		return Promise.reject(
			new Error(`Passed value for transition ${this} isn't an element`)
		);
	}
	if (!options) options = {};
...
function collapseElement(el, options) {
	...
	if (isNaN(desiredWidth))
		return Promise.reject(new Error("desired width must be a number"));
...
function moveAbsoluteElement(el, options) {
	...
	if (isNaN(xPos) || isNaN(yPos))
		return Promise.reject(new Error("desired position must be a number"));
...
function hideAndDo(el, options) {
	if (!options.callback)
		return Promise.reject(
			new Error("No callback passed por element: " + el.classList)
		);
...
function transitOpacity(el, options = {}, res) {
	...
	if (isNaN(desiredOpacity))
		return Promise.reject(
			new Error("Didn't passed desired opacity for transitioning element")
		);
...
function removeTransitionProperty(el, property) {
	let transitions = el.style.transition
		.match(/(((cubic-bezier\(.+?\))|(\w+[-.]?)+)\s?)+/g)
		.map((s) => s.trim()); //Prevent cubic-brezier to break code
	let updatedTransitions = removeFirstOccurrence(transitions, property);
	if (arraysAreIdentical(transitions, updatedTransitions)) {
		console.warn(
			`can't remove ${property} transition on ${el.tagName}: ${el.classList.item(0)}`
		);
		return;
	}
...
function addTransition(el, properties, options = {}) {
	...
	let newTransitions = properties.map((property) => {
		...
		if (
			propertyOptions.duration !== undefined &&
			propertyOptions.duration !== ""
		) {
			newTransition += ` ${propertyOptions.duration}`;
		}

		if (
			propertyOptions.timingFunction !== undefined &&
			propertyOptions.timingFunction !== ""
		) {
	...
	if (el.style.transition) {
		let existingTransitions = el.style.transition
			.split(",")
			.map((t) => t.trim());
...
function applyDefaultOptions(el, property, options) {
	...
	if (!options.disableCSS) {
		...
		if (index == -1) {
			console.warn(
				"can't apply default styles to a property that doesn't exists on element",
				el,
				property,
				options
			);
		...
		[
			...
			transitionTimingFunction,
			transitionDelay,
		] = transitionProperties.map((val) => {
			let defValues = [];

			if (val == transitionTimingFunction) {
				defValues = val.match(/(cubic-bezier\(.*\))|(\w+-?)+/g); //Prevent commas in cubic brezier to brake code
			}
			defValues = val.split(",").map((s) => s.trim());
			if (defValues != propLength)
				scaleArray(defValues, propLength, defValues[0]);
			return defValues[index];
		});

		const numberRegex = /[\d.]+/;
		//Check if value isn't configured and if number is greater than 0
		if (!optionsCopy.duration && transitionDuration.match(numberRegex)[0] > 0)
			optionsCopy.duration = transitionDuration;
		if (!optionsCopy.timingFunction)
			optionsCopy.timingFunction = transitionTimingFunction;
		if (!optionsCopy.delay && transitionDelay.match(numberRegex)[0] > 0)
			optionsCopy.delay = transitionDelay;
	}
