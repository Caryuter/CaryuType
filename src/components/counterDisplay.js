...
function updateCounter(settings, value) {
	...
	if (settings.gameMode == "time") {
		if (!value.timerCount && isNaN(value.timerCount))
			throw new Error("Can't set counter value without timer count");
		...
		if (
			...
			throw new Error(
				"Can't set counter value without words count or text length"
			);
